...
const SynchronizedReconciliationPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'details' | 'analytics' | 'sync'>(
    'overview'
  );
  ...
  const handleRecordUpdated = useCallback((update: RealTimeUpdate) => {
    ...
    setState((prev) => ({
      ...
      records: prev.records.map((record) =>
        record.id === update.recordId ? { ...record, ...update.details.updates } : record
      ),
    }));
  }, []);
  ...
  const processIndonesianMatches = async (records: EnhancedReconciliationRecord[]) => {
    ...
    const expenses = records.map(r => r.sources[0].data).filter((d: unknown) => (d as { type?: string }).type === 'expense')
    const bankRecords = records.map(r => r.sources[0].data).filter((d: unknown) => (d as { type?: string }).type === 'bank')
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (record) =>
          ...
          record.metadata.tags.some((tag) => tag.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }
          ...
          <div className="flex items-center space-x-2">
            <div
              className={`px-3 py-1 rounded-full text-sm font-medium ${
                state.synchronizationStatus === 'synchronized'
                  ? 'bg-green-100 text-green-800'
                  : state.synchronizationStatus === 'pending'
                    ? 'bg-yellow-100 text-yellow-800'
                    : state.synchronizationStatus === 'error'
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">
                {state.records.length > 0
                  ? (
                      (state.records.filter((r) => r.status === 'matched').length /
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-600">
                {state.records.length > 0
                  ? (
                      state.records.reduce((sum, r) => sum + r.confidence, 0) / state.records.length
                    ).toFixed(1)
                  : 0}
                %
              </p>
          ...
          <table className="w-full">
            ...
            <tbody>
              {paginatedRecords.map((record) => (
                <tr
                  key={record.id}
                  className={`border-b border-secondary-100 hover:bg-secondary-50 ${selectedRecords.has(record.id) ? 'bg-primary-50' : ''}`}
                >
                  ...
                  <td className="py-4 px-4">
                    <span
                      className={`px-2 py-1 text-xs font-medium rounded-full ${getRiskColor(record.riskLevel)}`}
                    >
        ...
        <div className="mt-6 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="text-sm text-secondary-500">
              ...
              {Math.min(pagination.page * pagination.pageSize, filteredAndSortedRecords.length)} of{' '}
