...
function loadLocal(name){
  ...
  const localPath = path.resolve(__dirname, 'cdp-runner', 'node_modules')
  const resolved = require.resolve(name, { paths: [localPath] })
  return require(resolved)
}
const chromeLauncher = loadLocal('chrome-launcher')
const CDP = loadLocal('chrome-remote-interface')
const fs = require('fs')

async function sleep(ms) { return new Promise(r => setTimeout(r, ms)) }

async function run() {
  // Ensure PATH includes standard bin paths for mktemp on macOS/Linux
  if (!process.env.PATH || !process.env.PATH.includes('/usr/bin')) {
    process.env.PATH = `/usr/bin:/bin:/usr/local/bin:${process.env.PATH || ''}`
  }
  ...
  try {
    ...
    await Promise.all([
      ...
      Log.enable(),
      // PerformanceTimeline may not be available or support longTask in all Chrome versions
      // Try to enable it, but don't fail if it's not supported
      PerformanceTimeline.enable({ eventTypes: ['longTask'] }).catch(() => {}),
    ])
    ...
    // Only subscribe to PerformanceTimeline if it was successfully enabled
    try {
      ...
      // PerformanceTimeline may not be available
    }
