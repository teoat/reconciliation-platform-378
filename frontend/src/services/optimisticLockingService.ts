...
class OptimisticLockingService {
  ...
  ): Promise<{ success: boolean; conflict?: ConflictResolution; data?: any }> {
    const lockKey = `${entityType}:${entityId}`
    const existingLock = this.findLockByKey(lockKey)
  ...
  private async createConflict(
    ...
    const conflict: ConflictResolution = {
      ...
      remoteData: existingLock ? { userId: existingLock.userId, version: existingLock.version } : null,
      resolution: 'manual',
  ...
  private findLockByKey(lockKey: string): OptimisticLock | undefined {
    return Array.from(this.locks.values()).find(lock => 
      `${lock.entityType}:${lock.entityId}` === lockKey
    )
  }
...
export const useOptimisticLocking = () => {
  ...
  const acquireLock = (entityType: string, entityId: string, userId: string, options?: any) => {
  ...
  const updateEntity = (entityType: string, entityId: string, userId: string, data: any, options?: any) => {
  ...
  const mergeData = (localData: any, remoteData: any, mergeStrategy: MergeStrategy) => {
