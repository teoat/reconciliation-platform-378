...
class MLMachingService {
  ...
  private cosineSimilarity(text1: string, text2: string): number {
    const tokenize = (text: string) => text.toLowerCase().split(/\s+/).filter(word => word.length > 2)
  ...
  private extractFeatures(recordA: Record, recordB: Record, config: MatchingConfig): Record<string, number> {
  ...
  private ensembleMatch(recordA: Record, recordB: Record, config: MatchingConfig): MatchCandidate {
    ...
    const explanation = explanations.length > 0
      ? explanations.join(', ')
      : 'Low similarity across all features'
  ...
  evaluateModel(modelId: string, testData: MatchCandidate[]): {
    accuracy: number
    precision: number
    recall: number
    f1Score: number
