...
class OfflineDataService {
  ...
  private loadStoredData(): void {
    try {
      ...
      logger.error('Failed to load offline data:', toRecord(error));
    }
  }
  ...
  private saveStoredData(): void {
    try {
      ...
      logger.error('Failed to save offline data:', toRecord(error));
    }
  }
  ...
  private async syncOfflineData(): Promise<void> {
    ...
    for (const data of dirtyData) {
      try {
        ...
        logger.error(`Failed to sync data ${data.id}:`, toRecord(error));
