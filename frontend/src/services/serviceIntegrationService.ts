...
class ServiceIntegrationService {
  ...
  private setupServiceListeners(): void {
    ...
    if (this.config.enableOfflinePersistence) {
      ...
      offlineDataService.on('recoveryPrompt', (prompt: unknown) => {
    ...
    if (this.config.enableOptimisticUI) {
      ...
      optimisticUIService.on('updateRolledBack', (data: unknown) => {
        this.emit('optimisticUpdateRolledBack', data);
      });

      optimisticUIService.on('conflictDetected', (conflict: unknown) => {
  ...
  public async handleError(
    ...
    try {
      ...
      if (this.config.enableOfflinePersistence) {
        offlineDataService.saveData(unifiedError.id, 'form', unifiedError, {
          projectId: options.projectId,
          userId: options.userId,
          component: options.component,
        });
  ...
  public async saveDataWithPersistence<T>(
    ...
    try {
      ...
      if (this.config.enableOfflinePersistence) {
        offlineDataService.saveData(options.id, options.type, data, {
          projectId: options.projectId,
          userId: options.userId,
          component: options.component,
...
export const useServiceIntegration = () => {
  ...
  const saveDataWithPersistence = <T>(
    data: T,
    options: {
      id: string;
      type: OfflineData['type'];
