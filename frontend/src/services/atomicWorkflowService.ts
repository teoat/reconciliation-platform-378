...
class AtomicWorkflowService {
  ...
  private loadPersistedData(): void {
    try {
      ...
      if (workflowsData) {
        ...
        data.workflows.forEach((workflowData: any) => {
          const workflow: WorkflowState = {
            ...
            locks: workflowData.locks.map((l: any) => ({
              ...l,
      ...
      if (operationsData) {
        ...
        data.operations.forEach((operationData: any) => {
  ...
  ): Promise<{ success: boolean; operation?: AtomicOperation; conflict?: any }> {
  ...
  private calculateProgress(workflow: WorkflowState): number {
    const stages = ['data_ingestion', 'data_mapping', 'reconciliation', 'review_results', 'export_results', 'completed']
...
export const useAtomicWorkflow = () => {
  ...
  const advanceWorkflow = (workflowId: string, toStage: string, userId: string, data?: any, options?: any) => {
