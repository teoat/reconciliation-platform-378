...
interface RecoveryStep {
  ...
  name: string
  description: string
  type: 'file_restore' | 'database_restore' | 'service_restart' | 'configuration_update' | 'validation'
  order: number
  timeout: number
  retryCount: number
  retryDelay: number
...
class BackupRecoveryService {
  ...
  // Recovery management
  public async createRecoveryPlan(plan: Omit<RecoveryPlan, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    const id = this.generateId()
    const now = new Date()
  ...
  public async executeRecovery(planId: string, reason: string, initiatedBy: string): Promise<string> {
  ...
  private async executeValidationStep(validation: ValidationStep, job: RecoveryJob): Promise<boolean> {
    job.logs.push({
      timestamp: new Date(),
      level: 'info',
