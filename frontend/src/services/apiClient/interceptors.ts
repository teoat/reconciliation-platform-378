...
export interface ResponseInterceptor {
  <T = unknown>(response: ApiResponse<T> | Response, config: RequestConfig): ApiResponse<T> | Response | Promise<ApiResponse<T> | Response>
}
...
export class InterceptorManager {
  ...
  async applyResponseInterceptors<T = unknown>(response: ApiResponse<T> | Response, config: RequestConfig): Promise<ApiResponse<T> | Response> {
...
export class AuthInterceptor {
  ...
  response<T = unknown>(response: ApiResponse<T> | Response, config: RequestConfig): ApiResponse<T> | Response {
    return response
  }
...
export class LoggingInterceptor {
  request(config: RequestConfig): RequestConfig {
    logger.log(`[API Request] ${config.method} ${config}`)
    return config
  }

  response<T = unknown>(response: ApiResponse<T> | Response, config: RequestConfig): ApiResponse<T> | Response {
    logger.log(`[API Response] ${config.method} ${config} - Success`)
    return response
  }
