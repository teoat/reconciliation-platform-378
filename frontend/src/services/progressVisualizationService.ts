...
class ProgressVisualizationService {
  ...
  private initializeContextualHelp(): void {
    ...
    this.contextualHelp.set('file_upload', {
      ...
      content: {
        ...
        description: 'Drag and drop your files here or click to browse. Supported formats: CSV, Excel',
        actions: [
          {
    ...
    this.contextualHelp.set('field_mapping', {
      ...
      content: {
        title: 'Field Mapping',
        description: 'Map your data fields to the system fields. Use AI suggestions for automatic mapping.',
        actions: [
          {
    ...
    this.contextualHelp.set('progress_indicator', {
      ...
      content: {
        ...
        description: 'Track your workflow progress. Click on any stage to see details and requirements.',
        actions: []
      }
    })
  }
  ...
  public getEstimatedTimeRemaining(): number {
    ...
    const remainingStages = workflow.filter(s => s.order >= currentStage.order && s.status === 'pending')
  ...
  private generateShortcuts(stage: WorkflowStage): { label: string; action: () => void; shortcut: string }[] {
...
export const useProgressVisualization = () => {
  ...
  const createProgressAnimation = (element: HTMLElement, progress: number, animation: ProgressAnimation) => {
