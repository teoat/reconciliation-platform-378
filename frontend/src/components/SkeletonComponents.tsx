  ...
  public showSkeleton(container: HTMLElement, skeletonType: 'table' | 'card' | 'list' | 'custom', config: any = {}): HTMLElement {
    ...
    switch (skeletonType) {
      ...
      case 'list':
        ...
        break
      case 'custom':
        skeleton = this.createCustomSkeleton(config.width || '100%', config.height || '100px', config)
        break
      default:
...
export const useSkeleton = () => {
  ...
  const createCustomSkeleton = (width: string, height: string, config?: Partial<SkeletonConfig>) => {
