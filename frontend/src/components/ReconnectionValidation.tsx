            ...
            {validationResult.inconsistencies.length > 0 && (
              <div className="space-y-3">
                ...
                {validationResult.inconsistencies.map((inconsistency, index) => (
                  <div key={index} className={`p-3 rounded-lg border ${getSeverityColor(inconsistency.severity)}`}>
...
export const ValidationSummary: React.FC<ValidationSummaryProps> = ({
  ...
  const getStatusColor = (): string => {
    ...
    if (validationResult.inconsistencies.some(i => i.severity === 'critical')) return 'text-red-600'
    if (validationResult.inconsistencies.some(i => i.severity === 'high')) return 'text-orange-600'
  ...
  const getStatusText = (): string => {
    ...
    if (validationResult.inconsistencies.some(i => i.severity === 'critical')) return 'Critical issues'
    if (validationResult.inconsistencies.some(i => i.severity === 'high')) return 'High priority issues'
    return 'Issues found'
  }
