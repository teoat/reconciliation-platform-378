...
export const ReconciliationInterface: React.FC<ReconciliationInterfaceProps> = ({
      ...
      setJobs((prevJobs) =>
        prevJobs.map((job) =>
          job.id === realtimeProgress.job_id
            ? {
                ...job,
                status: realtimeProgress.status as
                  | 'pending'
                  | 'running'
                  | 'completed'
                  | 'failed'
  ...
  const loadJobResults = useCallback(async (jobId: string, page = 1, perPage = 20) => {
    try {
      ...
      logger.error('Failed to load job results:', err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubscribeJobUpdate = subscribe('job_update', (data: Record<string, unknown> & { job_id?: string; project_id?: string; updates?: Record<string, unknown> }) => {
      if (data.job_id && data.project_id === projectId) {
        setJobs((prev) =>
          prev.map((job) => (job.id === data.job_id ? { ...job, ...data.updates } : job))
        );
      }
    });

    // Subscribe to progress updates
    const unsubscribeProgressUpdate = subscribe('job_progress', (data: Record<string, unknown> & { job_id?: string; progress?: ReconciliationProgress }) => {
