...
const AIDiscrepancyDetection = ({ project, onProgressUpdate }: AIDiscrepancyDetectionProps) => {
  const { getReconciliationData } = useData();
  ...
  const generateAIDetections = useCallback(
    (reconciliationData: ReconciliationData): AIDiscrepancyDetectionData[] => {
        ...
        if (record.status === 'discrepancy' && record.difference) {
          const detection: AIDiscrepancyDetectionData = {
            ...
            metadata: {
              ...
              context: {
                expectedRange: [
                  ((record.sources[0]?.data?.amount as number) || 0) * 0.95,
                  ((record.sources[0]?.data?.amount as number) || 0) * 1.05,
                ],
                actualValue: (record.sources[0]?.data?.amount as number) || 0,
                variance: record.sources[0]?.data?.amount
                  ? Math.abs(record.difference) / (record.sources[0]?.data?.amount as number)
                  : 0,
      ...
      // Analyze reconciliation records for high-value transactions
      reconciliationData.records.forEach((record, index: number) => {
        const amount = (record.sources[0]?.data?.amount as number) || 0;
        if (amount > 5000000) {
          // High value transaction
          const detection: AIDiscrepancyDetectionData = {
            id: `ai-detection-high-value-${index}`,
            type: 'amount',
            severity: 'medium',
            confidence: 87.3,
            description: `AI detected high-value transaction: ${(record.sources[0]?.data?.description as string) || 'Unknown'}`,
            sourceRecord: record.id,
            targetRecord: '',
            difference: 0,
      ...
      const nextWeekPredictions = [
        {
          ...
          output: {
            prediction: { type: 'expected', value: 'normal' },
            confidence: 0.89,
    ...
    const interval = setInterval(() => {
      setAnalysisProgress((prev) => {
        ...
        if (newProgress >= 100) {
          ...
          const aiDetections = generateAIDetections(reconciliationData);
  ...
  }, [getReconciliationData, onProgressUpdate, generateAIDetections, generateAIPredictions]);
            ...
            <div key={prediction.id} className="p-4 bg-secondary-50 rounded-lg">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-semibold text-secondary-900">
                    {typeof prediction.output.prediction === 'object'
