...
const MetricCard: React.FC<MetricCardProps> = memo(
  ({ title, value, change, icon, className = '', loading = false }) => {
    // Memoize change icon
    ...
    const changeClasses = useMemo(() => `flex items-center mt-2 ${changeColor}`, [changeColor]);
          ...
          <div className="flex-1">
            ...
            {change && (
              <div className={changeClasses}>
                {changeIcon}
                <span className="text-sm font-medium ml-1">{Math.abs(change.value)}%</span>
                {change.period && (
