...
export interface DataContextType {
  ...
  auditLogs: Array<{ id: string; userId: string; action: string; resource: string; result: 'success' | 'failure'; timestamp: string; details?: Record<string, unknown> }>;
  isSecurityEnabled: boolean;
  checkPermission: (userId: string, resource: string, action: string) => boolean;
  logAuditEvent: (userId: string, action: string, resource: string, result: 'success' | 'failure', details?: Record<string, unknown>) => void;
  encryptData: (data: unknown, dataType: string) => string;
  decryptData: (encryptedData: string, dataType: string) => unknown;
  checkCompliance: (framework: string) => Array<{ framework: string; status: string; issues: string[] }>;
  ...
  addReconciliationData: (projectId: string, reconciliationData: ReconciliationData) => ProjectData | null;
