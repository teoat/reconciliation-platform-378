...
export const useDataStorage = (
  ...
  const createProject = useCallback(
    (project: Partial<ProjectData>): ProjectData => {
      setIsLoading(true);
      setError(null);
  ...
  const updateProject = useCallback(
    (projectId: string, updates: Partial<ProjectData>): ProjectData | null => {
      setIsLoading(true);
      setError(null);
  ...
  const addIngestionData = useCallback(
    (projectId: string, ingestionData: IngestionData): ProjectData | null => {
      setIsLoading(true);
      setError(null);
  ...
  const addReconciliationData = useCallback(
    (projectId: string, reconciliationData: ReconciliationData): ProjectData | null => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to add reconciliation data';
        setError(errorMessage);
        return null;
      } finally {
        setIsLoading(false);
  ...
  const addCashflowData = useCallback(
    (projectId: string, cashflowData: CashflowData): ProjectData | null => {
      setIsLoading(true);
      setError(null);
  ...
  const transformIngestionToReconciliation = useCallback(
    (projectId: string): ProjectData | null => {
      setIsLoading(true);
      setError(null);
  ...
  const transformReconciliationToCashflow = useCallback(
    (projectId: string): ProjectData | null => {
      setIsLoading(true);
      setError(null);
  ...
  const subscribeToProject = useCallback(
    (projectId: string, callback: (data: ProjectData) => void) => {
