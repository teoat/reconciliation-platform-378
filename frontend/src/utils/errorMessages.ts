...
export const getUserFriendlyError = (error: Error | string): UserFriendlyErrorInfo => {
  ...
  if (errorMessage.includes('auth') || errorMessage.includes('unauthorized') || errorMessage.includes('401')) {
    return {
      ...
      recoveryActions: [
        { label: 'Go to Login', action: '/login' },
      ],
      severity: 'warning',
    };
  }
  ...
  // Validation errors
  if (errorMessage.includes('valid') || errorMessage.includes('required') || errorMessage.includes('invalid')) {
    return {
      title: 'Validation Error',
      message: 'Some information is missing or incorrect. Please review and correct the highlighted fields.',
      suggestions: [
...
export const getContextualError = (
  ...
  const context = component && action ? `in ${component} while ${action}` : component ? `in ${component}` : '';
