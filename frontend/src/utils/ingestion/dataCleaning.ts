    ...
    Object.keys(cleanedRecord).forEach((key) => {
      ...
      if (typeof value === 'string') {
        ...
        if (
          ...
          cleanedRecord[key] = (parseFloat(value.replace(/[^0-9.-]/g, '')) || 0).toString();
        }
  ...
  if (fileType === 'expenses') {
    return Array.from({ length: recordCount }, (_, i) => ({
      ...
      description: [
        ...
        'Marketing',
        'Utilities',
      ][Math.floor(Math.random() * 5)] || 'Misc',
      category: ['Administrative', 'Travel', 'Technology', 'Marketing', 'Operations'][
        Math.floor(Math.random() * 5)
      ] || 'Other',
      amount: (Math.random() * 5000 + 100).toFixed(2),
      vendor: ['Vendor A', 'Vendor B', 'Vendor C', 'Vendor D'][Math.floor(Math.random() * 4)] || 'Vendor X',
      reference: `REF-${Math.floor(Math.random() * 10000)}`,
      status: ['Approved', 'Pending', 'Rejected'][Math.floor(Math.random() * 3)] || 'Pending',
    }));
    ...
    return Array.from({ length: recordCount }, (_, i) => ({
      ...
      type: ['Credit', 'Debit'][Math.floor(Math.random() * 2)] || 'Debit',
    ...
    return Array.from({ length: recordCount }, (_, i) => ({
      ...
      field3: ['Option A', 'Option B', 'Option C'][Math.floor(Math.random() * 3)] || 'Option A',
