 ...
 */
export function extractNumber(data: unknown, field: string, defaultValue: number): number {
 ...
 */
export function extractString(data: unknown, field: string, defaultValue: string): string {
  if (!isSourceData(data)) return defaultValue;
  const value = data[field];
