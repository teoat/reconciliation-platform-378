...
export const useSecurity = () => {
  ...
  const decryptData = useCallback((encryptedData: Record<string, unknown> & { _encrypted?: boolean }, dataType: string): Record<string, unknown> => {
  ...
  const checkCompliance = useCallback((framework: string): ComplianceRequirement[] => {
  ...
  const createSecurityPolicy = useCallback((policy: Omit<SecurityPolicy, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  const updateSecurityPolicy = useCallback((policyId: string, updates: Partial<SecurityPolicy>) => {
  ...
  const deleteSecurityPolicy = useCallback((policyId: string) => {
  ...
  // Export audit logs
  const exportAuditLogs = useCallback((format: 'csv' | 'json' = 'json') => {
