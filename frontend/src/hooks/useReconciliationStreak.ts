...
export function useReconciliationStreak(userId: string) {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        logger.error('Failed to load streak data:', toRecord(error));
      }
    }
  }, [userId]);
  ...
  const checkStreakStatus = (currentStreak: StreakData) => {
    const now = new Date();
    const lastDate = currentStreak.lastReconciliationDate
      ? new Date(currentStreak.lastReconciliationDate)
      : null;
