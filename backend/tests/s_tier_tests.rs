...
use reconciliation_backend::services::{
    ...
    security_monitor::{AnomalyDetectionConfig, SecurityMonitor},
...
async fn test_security_monitor_brute_force_detection() {
    ...
    let detected = monitor
        .detect_brute_force("127.0.0.1", false)
        .await
        .unwrap();
    assert!(
        detected,
        "Should detect brute force attack after 5 failures"
    );
}
...
async fn test_query_optimizer_suggestions() {
    ...
    let analysis = optimizer
        .analyze_query("SELECT * FROM users", Duration::from_millis(50))
        .await;

    let has_select_star_warning = analysis
...
async fn test_query_optimizer_stats() {
    ...
    optimizer
        ...
        .await;
    optimizer
        .analyze_query("SELECT * FROM orders", Duration::from_millis(200))
        .await;
