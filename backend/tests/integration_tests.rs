...
mod api_integration_tests {
    ...
    async fn test_reconciliation_flow() {
        ...
        // Upload files
        let file1_id = test_client
            .upload_file(&project_id, "./test_data/source1.csv")
            .await
            .unwrap();
        let file2_id = test_client
        ...
        let job_id = test_client
            .create_reconciliation_job(&project_id, "Test Reconciliation")
            .await
            .unwrap();
    ...
    async fn test_analytics_endpoints() {
        ...
        // Get dashboard data
        let req = test_client
            .authenticated_request("GET", "/api/analytics/dashboard")
            .await;
        let resp = test::call_service(&test_client.app, req).await;
        assert!(resp.status().is_success());
        ...
        let req = test_client
            .authenticated_request("GET", "/api/analytics/reconciliation")
            .await;
        let resp = test::call_service(&test_client.app, req).await;
        assert!(resp.status().is_success());
    ...
    async fn test_user_management_endpoints() {
        ...
        let req = test_client
            .authenticated_request("POST", "/api/users")
            .set_json(&user_data);
        let resp = test::call_service(&test_client.app, req).await;
        assert!(resp.status().is_success());
        ...
        // Get user details
        let req = test_client
            .authenticated_request("GET", &format!("/api/users/{}", user_id))
            .await;
        let resp = test::call_service(&test_client.app, req).await;
        assert!(resp.status().is_success());
        ...
        let req = test_client
            .authenticated_request("PUT", &format!("/api/users/{}", user_id))
            .set_json(&update_data);
        let resp = test::call_service(&test_client.app, req).await;
        assert!(resp.status().is_success());
        ...
        // Delete user
        let req = test_client
            .authenticated_request("DELETE", &format!("/api/users/{}", user_id))
            .await;
        let resp = test::call_service(&test_client.app, req).await;
        assert!(resp.status().is_success());
    }
...
mod middleware_integration_tests {
    ...
    async fn test_security_middleware() {
        ...
        let app = test::init_service(
            App::new()
                ...
                .route(
                    "/test",
                    web::get().to(|| async { HttpResponse::Ok().json("test") }),
                ),
        )
        .await;
    ...
    async fn test_auth_middleware() {
        ...
        let app = test::init_service(App::new().wrap(AuthMiddleware::new(auth_config)).route(
            "/protected",
...
mod database_integration_tests {
    ...
    async fn test_database_connection() {
        let test_db =
            TestDatabaseManager::new("postgresql://test_user:test_pass@localhost:5432/test_db");
