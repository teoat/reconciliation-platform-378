...
use reconciliation_backend::services::reconciliation::{
    CreateReconciliationJobRequest, MatchingAlgorithm, ReconciliationJob, ReconciliationResult,
...
mod reconciliation_job_tests {
    ...
    async fn test_create_and_start_job() {
        ...
        let job = reconciliation_service
        ...
        assert_eq!(
            updated_job.status,
            "running" || updated_job.status == "processing"
        );
    }
