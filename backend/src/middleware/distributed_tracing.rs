...
mod tests {
    ...
    async fn test_span_tagging() {
        ...
        if let Err(e) = tracing
            .add_tag(&span_id, "key".to_string(), "value".to_string())
            .await
        {
    ...
    B: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let span_id = tracing
                .start_span(&format!("{} {}", method, path), None)
                .await;

            // Add tags
            tracing
                .add_tag(&span_id, "http.method".to_string(), method.clone())
                .await?;
            tracing
                .add_tag(&span_id, "http.path".to_string(), path.clone())
                .await?;
            tracing
                .add_tag(
                    &span_id,
                    "service.name".to_string(),
                    tracing.config.service_name.clone(),
                )
                .await?;
            ...
            match result {
                ...
                Err(error) => {
                    tracing
                        .add_tag(&span_id, "error".to_string(), "true".to_string())
                        .await?;
                    tracing
                        .add_tag(&span_id, "error.message".to_string(), error.to_string())
                        .await?;
                    tracing.finish_span(&span_id).await?;
                    Err(error)
                }
