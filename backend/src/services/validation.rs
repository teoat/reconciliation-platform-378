...
impl ValidationService {
    pub fn new() -> Self {
        Self {
            ...
            password_regex: Regex::new(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$").unwrap(),
            file_extension_regex: Regex::new(r"^\.(csv|xlsx|xls|json|xml|txt)$").unwrap(),
        }
    }
        ...
        if let Some(properties) = schema.get("properties") {
            if let Some(properties_obj) = properties.as_object() {
                for (key, prop_schema) in properties_obj {
                    ...
                    } else if schema.get("required").and_then(|r| r.as_array()).map_or(false, |r| r.contains(&serde_json::Value::String(key.clone()))) {
