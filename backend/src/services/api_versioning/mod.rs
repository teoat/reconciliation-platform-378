...
mod tests {
    ...
    async fn test_api_versioning() {
        ...
        let compatibility = service
            .check_client_compatibility("1.0.0", "1.0.0")
            .await
            .expect("Failed to check compatibility");
        assert!(compatibility.is_compatible);

        let endpoint_version = service
            .get_endpoint_version("GET", "/api/users")
            .await
            .expect("Failed to get endpoint version");
        assert!(endpoint_version.is_some());

        let supported = service
            .get_supported_versions("GET", "/api/users")
            .await
        ...
        assert!(service
            ...
            .expect("Failed to check version satisfies"));
        assert!(!service
    ...
    async fn test_migration_strategies() {
        ...
        service
            ...
            .await
            .expect("Failed to add migration strategy");

        let retrieved = service
            .get_migration_strategy("1.0.0", "2.0.0")
            .await
            .expect("Failed to get migration strategy");
        assert!(retrieved.is_some());

        let strategies = service
            .list_migration_strategies()
            .await
            .expect("Failed to list migration strategies");
    ...
    async fn test_version_management() {
        ...
        service
            ...
            .await
            .expect("Failed to deprecate version");
        ...
        service
            ...
            .await
            .expect("Failed to sunset version");
        ...
        let stats = service
            .get_version_stats()
            .await
            .expect("Failed to get version stats");
