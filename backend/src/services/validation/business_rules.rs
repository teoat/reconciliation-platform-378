...
impl BusinessRulesValidator {
    ...
    fn validate_user_business_rules(
        &self,
        data: &HashMap<String, serde_json::Value>,
    ) -> AppResult<()> {
    ...
    fn validate_project_business_rules(
        &self,
        data: &HashMap<String, serde_json::Value>,
    ) -> AppResult<()> {
    ...
    fn validate_reconciliation_job_business_rules(
        &self,
        data: &HashMap<String, serde_json::Value>,
    ) -> AppResult<()> {
    ...
    fn validate_data_source_business_rules(
        &self,
        data: &HashMap<String, serde_json::Value>,
    ) -> AppResult<()> {
        if let Some(source_type) = data.get("source_type") {
            if let Some(type_str) = source_type.as_str() {
                let valid_types = ["csv", "xlsx", "xls", "json", "xml", "database"];
                if !valid_types.contains(&type_str) {
                    return Err(AppError::Validation(format!(
                        "Invalid source type: {}. Valid types are: {}",
                        type_str,
                        valid_types.join(", ")
                    )));
                }
