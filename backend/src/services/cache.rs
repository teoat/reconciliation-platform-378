...
impl MultiLevelCache {
    pub fn new(redis_url: &str) -> AppResult<Self> {
        ...
        let encoded_url = if redis_url.contains("://:") && redis_url.contains("@") {
            // Extract and encode password from redis://:password@host:port format
            if let Some(at_pos) = redis_url.find('@') {
                if let Some(password_start) = redis_url.find("://:") {
                    let password_end = at_pos;
                    let password = &redis_url[password_start + 4..password_end];
                    let encoded_password = urlencoding::encode(password);
                    let new_url = format!(
                        "{}{}{}",
                        &redis_url[..password_start + 4],
                        encoded_password,
                        &redis_url[password_end..]
                    );
                    new_url
                } else {
                    redis_url.to_string()
        ...
        let l2_cache = CacheService::new(&encoded_url)?;
        ...
        T: for<'de> Deserialize<'de> + Clone + Serialize,
    {
        ...
        if let Some(value) = self.l1_cache.read().await.get(key) {
            if !value.is_expired() {
                ...
                let deserialized: T = serde_json::from_value(value.data.clone()).map_err(|e| {
                    AppError::InternalServerError(format!(
                        "Failed to deserialize L1 cache value: {}",
                        e
                    ))
                })?;
        ...
        T: Serialize,
    {
        ...
        l1_cache.insert(key.to_string(), CachedValue::new(json_value, ttl));
    ...
    async fn evict_oldest_entries(
        &self,
        cache: &mut HashMap<String, CachedValue<serde_json::Value>>,
    ) {
