...
impl AnalyticsProcessor {
    ...
    pub fn process_recent_activity(&self, activities: Vec<ActivityItemQueryResult>) -> Vec<ActivityItem> {
        activities
            .into_iter()
            .map(|result| {
                ActivityItem {
    ...
    pub fn process_performance_metrics(&self, conn: &mut diesel::PgConnection) -> AppResult<PerformanceMetrics> {
    ...
    pub fn get_project_confidence_score(&self, project_id: Uuid, conn: &mut diesel::PgConnection) -> AppResult<f64> {
        let avg_confidence = reconciliation_results::table
    ...
    pub fn get_project_match_counts(&self, project_id: Uuid, conn: &mut diesel::PgConnection) -> AppResult<(i64, i64)> {
    ...
    pub fn get_reconciliation_metrics(&self, conn: &mut diesel::PgConnection) -> AppResult<(i64, f64, f64)> {
