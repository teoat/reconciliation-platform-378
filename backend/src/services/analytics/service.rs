...
use crate::models::schema::{audit_logs, reconciliation_jobs, users};
...
impl AnalyticsService {
    pub fn new(db: Database) -> Self {
        ...
        let cache = Arc::new(MultiLevelCache::new(&redis_url).unwrap_or_else(|e| {
            log::error!(
                "Failed to initialize cache with Redis URL {}, falling back to localhost: {}",
                redis_url,
                e
            );
            MultiLevelCache::new("redis://localhost:6379").unwrap_or_else(|fallback_err| {
                ...
                panic!(
                    "Cannot initialize cache: both primary and fallback Redis connections failed"
                );
            })
        }));
    ...
    pub async fn get_dashboard_data(&self) -> AppResult<DashboardData> {
        ...
        if let Some(ref resilience) = self.resilience {
            if let Some(cached_data) = resilience
                .execute_cache(async { self.cache.get::<DashboardData>(cache_key).await })
                .await?
            {
                return Ok(cached_data);
            }
        } else if let Some(cached_data) = self.cache.get::<DashboardData>(cache_key).await? {
            return Ok(cached_data);
        }
    ...
    pub async fn get_project_stats(&self, project_id: Uuid) -> AppResult<ProjectStats> {
        ...
        if let Some(ref resilience) = self.resilience {
            if let Some(cached_stats) = resilience
                .execute_cache(async { self.cache.get::<ProjectStats>(&cache_key).await })
                .await?
            {
                return Ok(cached_stats);
            }
        } else if let Some(cached_stats) = self.cache.get::<ProjectStats>(&cache_key).await? {
            return Ok(cached_stats);
        }
        ...
        // Get match counts
        let (total_matches, total_unmatched) = self
            .processor
            .get_project_match_counts(project_id, &mut conn)?;

        // Get average confidence score
        let avg_confidence = self
            .processor
            .get_project_confidence_score(project_id, &mut conn)?;
    ...
    pub async fn get_user_activity_stats(&self, user_id: Uuid) -> AppResult<UserActivityStats> {
        ...
        let (total_actions, projects_created, jobs_created, files_uploaded) = self
            .collector
            .get_user_activity_counts(user_id, &mut conn)?;
        ...
        let activity_dates = self
            .collector
            .get_user_daily_activity_dates(user_id, &mut conn)?;
