...
impl ShardConfig {
    pub fn create_default_shards() -> Vec<Self> {
        let base_url = std::env::var("DATABASE_URL")
            .unwrap_or_else(|_| "postgres://postgres:postgres_pass@postgres:5432/reconciliation_app".to_string());
        ...
        let shards = vec![
            ...
            ShardConfig {
                ...
                connection_string: format!("{}?sslmode=disable", base_url.replace("reconciliation_app", "reconciliation_shard_1")),
                user_range_min: 170141183460469231731687303715884105727 / 4 + 1,
                user_range_max: 170141183460469231731687303715884105727 / 2,
            ...
            ShardConfig {
                ...
                connection_string: format!("{}?sslmode=disable", base_url.replace("reconciliation_app", "reconciliation_shard_2")),
                user_range_min: 170141183460469231731687303715884105727 / 2 + 1,
                user_range_max: (170141183460469231731687303715884105727 * 3) / 4,
            ...
            ShardConfig {
                ...
                connection_string: format!("{}?sslmode=disable", base_url.replace("reconciliation_app", "reconciliation_shard_3")),
                user_range_min: (170141183460469231731687303715884105727 * 3) / 4 + 1,
...
impl ShardManager {
    pub fn new(shard_configs: Vec<ShardConfig>) -> Self {
        let shard_map = shard_configs.iter()
...
impl ShardedPoolManager {
    pub fn new(shard_configs: Vec<ShardConfig>) -> Result<Self, String> {
        ...
        for shard_config in shard_configs {
            ...
            let pool = Pool::builder()
                ...
                .min_idle(Some(1))
                .build(manager)
                .map_err(|e| format!("Failed to create pool for shard {}: {}", shard_config.id, e))?;
    ...
    pub async fn get_pool_for_user(&self, user_id: Uuid) -> Result<Arc<Mutex<PgPool>>, String> {
        let shard = self.shard_manager.get_shard_for_user(user_id)
            .ok_or_else(|| "No shard found for user".to_string())?;

        self.pools.get(&shard.id)
            .cloned()
            .ok_or_else(|| format!("No pool found for shard {}", shard.id))
    }

    /// Get a connection for a specific user
    pub async fn get_connection_for_user(&self, user_id: Uuid) -> Result<diesel::r2d2::PooledConnection<ConnectionManager<PgConnection>>, String> {
