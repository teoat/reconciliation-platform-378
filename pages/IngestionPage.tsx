...
type DataRow = Record<string, ColumnValue> & {
  id: string;
};
...
const IngestionPage = ({ project }: IngestionPageProps) => {
  ...
  const validateData = (data: DataRow[], fileType: string): DataValidation[] => {
    ...
    columns.forEach((column) => {
      ...
      if (column.toLowerCase().includes('date')) {
        const invalidDates = values.filter((val) => {
          ...
          return true; // non-string/number is invalid
        });
      ...
      if (column.toLowerCase().includes('email')) {
        ...
        const invalidEmails = values.filter(
          (val) => typeof val === 'string' && !emailRegex.test(val)
        );
  ...
  const analyzeDataQuality = (data: DataRow[]): DataQualityMetrics => {
  ...
  const inferColumnTypes = (data: DataRow[]): ColumnInfo[] => {
