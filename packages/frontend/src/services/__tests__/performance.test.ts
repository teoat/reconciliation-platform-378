...
describe('Form Service Performance Tests', () => {
  ...
  it('should manage multiple auto-save sessions', async () => {
    const { duration } = await measurePerformance(async () => {
      for (let i = 0; i < 50; i++) {
        formService.startAutoSave(`form_${i}`, { data: `value_${i}` }, {
          page: 'test-page',
          userId: 'user-123'
        })
      }
...
describe('Memory Usage Tests', () => {
  it('should not leak memory with form service', async () => {
    ...
    for (let i = 0; i < 1000; i++) {
      const id = formService.saveFormData(`form-${i}`, { data: `value-${i}` }, {
        page: 'test-page',
        userId: 'user-123'
      })
...
describe('Concurrent Operations Tests', () => {
  it('should handle concurrent form operations', async () => {
    const operations = Array.from({ length: 100 }, (_, i) => 
      formService.saveFormData(`form-${i}`, { data: `value-${i}` }, {
        page: 'test-page',
        userId: 'user-123'
      })
    )
