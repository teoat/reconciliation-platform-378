/**
 * Frenly Guidance Agent - Core Implementation
 */
...
import { MessageContext, GeneratedMessage } from './frenly-interfaces';
import {
  getHelpContentService,
  getOnboardingService,
  getAIService,
  getNLUService,
  getMCPIntegrationService,
} from './frenly-services';
import { BehaviorTracker } from './frenly-behavior';
...
export class FrenlyGuidanceAgent implements MetaAgent {
  ...
  private behaviors = new BehaviorTracker();
  ...
  private readonly messageCache: Map<string, { message: GeneratedMessage; accessTime: number }> =
    new Map();
  ...
  async cleanup(): Promise<void> {
    ...
    this.behaviors.cleanupOldBehaviors();
  ...
  async generateMessage(context: MessageContext): Promise<GeneratedMessage> {
    ...
    const requestPromise = (async (): Promise<GeneratedMessage> => {
      try {
        ...
        const behavior = this.behaviors.getUserBehavior(context.userId);
  ...
  private async determineMessageType(
    ...
    try {
      ...
      logger.debug('Error checking onboarding progress:', {
  ...
  private async generateContent(
    ...
    try {
      ...
      logger.debug('MCP integration failed, continuing without insights:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
    ...
    try {
      ...
      logger.debug('AI generation failed, using fallback:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
...
- Provides actionable guidance${mcpInsight ? `\n- Incorporates system insight: ${mcpInsight}` : ''}`;
      ...
      logger.debug('AI generation error:', {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  private async generateTipWithHelp(context: MessageContext, style: string): Promise<string> {
    ...
    try {
      ...
      logger.debug('Error fetching help content:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
    ...
    try {
      ...
      if (context.userId) {
        ...
        if (onboardingProgress && onboardingProgress.currentStep) {
          ...
          const searchResults =
            helpService?.search(onboardingProgress.currentStep as string, 1) || [];
      ...
      logger.debug('Error fetching help content for guidance:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
   ...
   */
  private determinePriority(context: MessageContext, _behavior: any): GeneratedMessage['priority'] {
    if (context.behavior?.errors && context.behavior.errors > 0) {
      return 'high';
    }
  ...
  async handleUserQuery(
    ...
    try {
      ...
      const behavior = this.behaviors.getUserBehavior(userId);
      ...
      const understanding = await nlu.understand(query, {
        ...
        page: context?.page,
        progress: context?.progress,
        recentActions: behavior.messageInteractions.slice(-5).map((i) => i.messageId),
      });
      ...
      logger.error('Error handling user query:', {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async recordFeedback(
    ...
    return this.behaviors.recordFeedback(userId, messageId, feedback);
  }
  ...
  async trackInteraction(userId: string, action: string, messageId?: string): Promise<void> {
    return this.behaviors.trackInteraction(userId, action, messageId);
  }
